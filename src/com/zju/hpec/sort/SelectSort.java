 ...
 * @Company:
 * @author summer 2016年4月10日
 *
 */
public class SelectSort {

	/**
	 * directly sort 1. Firstly,select the index of the minimum or the maximum
	 * element in the disordered array 2. Secondly,swap the first or last
	 * element in the disordered array and the selected minimum or maximum
	 * element
	 * 
	 * @param array
	 */
	...
	public static void heapSort(int[] array) {
		...
		for (int i = 0; i < array.length; ++i) {
			...
			// the maximum element of the array will be set to the first
			// position
	 ...
	 * @param array
	 * @param last
	 *            the last position of the array will be used to build heap
	 */
	private static void buildMaxHeap(int[] array, int last) {
		for (int i = (last - 1) / 2; i >= 0; --i) {
			...
			while (k * 2 + 1 < last) {
				...
				// find the maximum value between the two children nodes of the
				// current node
				...
				// swap the parent and the child node if the judge statement is
				// true
	...
	private static void buildMinHeap(int[] array, int last) {
		for (int i = (last - 1) / 2; i >= 0; --i) {
			...
			while (k * 2 + 1 < last) {
				int minIndex = k * 2 + 1;
				// find the minimum value between the two children nodes of the
				// current node
				// and set the index to the maxIndex variable
				if (minIndex < last) {
